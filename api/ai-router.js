...
async function generateWithFallback(prompt, options = {}) {
  ...
  for (const provider of providers) {
    for (let attempt = 0; attempt <= retries; attempt++) {
      try {
        ...
        console.log(`Provider ${provider.name} attempt ${attempt + 1} failed: ${err.message}`);
        if (attempt < retries) {
  ...
  return { text: 'AI services unavailable. Please check API keys.', provider: 'fallback', warning: true };
}
